...
The paper is structured as follows. Section \@ref(sec:str) reviews temporal data structures corresponding to time series and longitudinal analysis, and discusses "tidy data". Section \@ref(sec:semantics) proposes contextual semantics for temporal data, built on top of tidy data principles. The concept of data pipelines, with respect to the time domain, is discussed in depth in Section \@ref(sec:pipeline), followed by a discussion of the design choices made in the software implementation in Section \@ref(sec:software). Two case studies are presented in Section \@ref(sec:cases) illustrating temporal data exploration using the new infrastructure. Section \@ref(sec:conclusion) summarises current work and discusses future directions.
