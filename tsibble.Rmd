...
The **tsibble** package not only provides a "tsibble" data object but also a domain specific language in R for transforming temporal data. It takes advantage of the wrangling verbs implemented in the **dplyr** package, and develops a suite of new tools for facilitating temporal manipulation for easing, primarily, two aspects: implicit missingness handlers and time-aware aggregations.
...
Modeling is crucial to explanatory and predictive analytics, where time series and longitudinal data analysis diverges. The tsibble, as a model-oriented object, can flow into both types of modelling, and the new semantics (index and key) can be internally utilized to accelerate modelling.
