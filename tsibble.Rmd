...
This paper describes a new tidy representation to encompass both time series and longitudinal data, and a unified framework to streamline the workflow from data preprocessing to visualization and forecasting, as an integral part of a tidy data analysis.
...
The principal divide of temporal data is regularly versus irregularly spaced data. Event data typically involves irregular time intervals, such as flight schedules or customer transactions. This type of data can flow into event-based data modeling, but would need to be processed or regularized to fit models that expect data with a fixed interval. When data indexed in regular time space, tsibble computes the greatest common divisor as the time interval from the index, or an unknown interval if one observation is available for each unit.
...
Panel data models, however, put emphases on overall, within, and between variations both across individuals and time. Fixed and random effects models can be developed in line with the **fable** tools.
...
All user-facing functions can be roughly organized into three major chunks according to their functionality: vector functions (1d), table verbs (2d), and window family. Each chunk is an independent module, but works interdependently. Vector functions in the package mostly deal with time. The atomic functions (such as `yearmonth()` and `yearquarter()`) embedded in the `index_by()` table verb collapses a tsibble to a less granular interval. The substitution of another time function in the `index_by()` results in the aggregation of different time resolution. Since these time functions are not exclusive to a tsibble, they can be used in a variety of applications in conjunction with other packages. The extensible design of the tsibble package means that the table verbs can incorporate many third-party vector functions. It is also generally easier to trace back the errors users encounter from separating 1d and 2d functions, and increase the code readability.
...
```{r write-bib}
write_bib(c("dplyr", "tidyr", "purrr", "nanotime", "tibbletime", "tsibble", "magrittr", "future", "furrr", "bomrang", "rlang"), file = "rpkgs.bib")
```
