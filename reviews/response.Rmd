...
1. Section 2.2 (previously 2.3) reviews existing time series and longitudinal data standards in popular statistical software, such as R, Stata, and SAS.
...
We have added a code snippet (featuring new `pedestrian` dataset) in a new subsection 4.4, and the load forecasting example to the energy consumption case study to show the cleaner, more expressive temporal data pipeline.
...
We'd like to upgrade `spread`/`gather` verbs to `pivot_longer`/`pivot_wider`. But they are not S3 generics yet, which delays the implementation. Conceptually the newly melt columns will be added to the key specification.
...
Time indexes lay the contextual foundation to both time series and longitudinal data. `zoo` and `xts` store indices as metadata due the matrix structure, but tsibble's indices are explicitly a data column that can be manipulated. Tsibble also has the capacity of custom index support and interval extraction.

Tsibble is aimed at general temporal data, to unify time series and longitudinal **data**. In the revised manuscript (Section 4), we note the focuses of time series and longitudinal **analysis** are different in the modelling practice. Yes, the presented case studies are time series oriented, because currently time series model functions are available for tsibble.

> Figure 4 animation doesn't play so authors may like to ensure that the final publication works. Github version works.

The JCGS peer-reviewed version is processed with watermark, which prevents the animation playing.
...
We have added the sentence to Section 3.3, "Although tsibble reports one single interval despite data of fixed or mixed intervals, data of mixed intervals should be organized in separate tsibbles instead for well-tailored analysis and models.", to address the mixed time resolutions.
