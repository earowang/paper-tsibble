...
Besides these major changes, we have done lots of minor tweaks to improve the manuscript.
...
We'd like to upgrade `spread`/`gather` verbs to `pivot_longer`/`pivot_wider`. But they are not S3 generics yet, and embedding tsibble into the new vctrs framework will take a long time, which delays the implementation. Conceptually the newly melt columns will be added to the key specification.
