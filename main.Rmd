...
(ref:count-gaps-cap) Time gap plots for the 49 customers with most implicit missing values, and the remaining customers grouped into the one line at top. Each cross represents a missing in time and a line between two dots shows continuous missingness over time. Each row corresponds to one customer. Missing values occur at various times, with perhaps more in January.

Aggregation across all individuals helps to sketch a big picture about the representative behaviors change over time, organized into a calendar display (Figure \@ref(fig:calendar-plot)). Each glyph represents the daily pattern of residential electricity usage every thirty minutes.  Higher consumption is indicated by higher values, and typically occurs in daylight hours. Colour indicates hot days. The daily snapshots vary depending on the season in the year. During the summer months (December and January), the late-afternoon peak becomes predominant driven by the use of air conditioning, especially in the hot days with daily average temperature greater than 25 degrees. However, the winter time (July and August) sees two peaks in a day, which is probably due to heating in the morning and evening. This plot illustrates how the tsibble data can easily integrate with other tools and graphics.

```{r calendar-plot, fig.height = 7, fig.cap = "(ref:calendar-plot-cap)"}
```

(ref:calendar-plot-cap) Half-hourly average electricity use across all customers in the region, organized into calendar format, with colour indicating hot days. Energy use of hot days tends to be higher, suggesting air conditioner use. Days in the winter months have a double peak suggesting heater use.
...
A new data abstraction to represent temporal data named as "tsibble" has been presented, spotlighting the "tidy data" principles brought to time domain. Tidy data begins to take shape in the state of time with the introduction of contextual semantics: index and key. A declared index provides direct support to time variable; variables that comprise the key define observations over time. These semantics further determine unique data entries required for a valid tsibble. No matter how temporal data arrives, tsibble respects a time index and mainatins the data richness. A tsibble frictionlessly pops into transformation, visualization, modelling and smoothly shifts between, allowing for rapid iteration to gain data insights.

A missing piece of the *tsibble* data is to enable user-defined calendars and respect structural missing observations. For example, a call center operates only between 9:00am and 5:00pm on week days and stock trading resumes on Monday straight after Friday. No data available outside trading hours would be labelled as structural missingess, which *tsibble* currently disregards. However, few R packages provide functionality to create and manage many sorts of calendars, including market-specific business calendar. <!-- This delays the implementation.--> Generally, custom calendars would be easily embedded into the tsibble framework. Consequently these tsibble operators, like `fill_gaps()`, would work out of box, and forecasts would be generated within its definable time range.
