...
The data easily fits into the tsibble framework without the loss of data richness and heterogeneity. To declare it as a tsibble, the setup is to specify `sched_dep_datetime` as "index", `flight` and `origin` variables as "key", and `regular = FALSE`. The observation unit is defined by a flight at its origin. A rigorous check is performed for validating the distinct rows determined by key and index.

```{r tsibble, echo = TRUE}
```

On the basis of the tsibble, what question to be asked leads its way to composing a time-based pipeline in order for exploratory data analysis in time context. For example, one would be interested in the number of flights between origins and destinations every day, daily proportions of delayed flights by carriers, and etc. In this section, what time of the day and day of the week is good for travelling in order to avoid severe delay, from passenger's point of view? A given pipeline helps explore and demonstrate the distribution of departure delay conditional on time of day and day of week, particularly the upper tail behaviors. The accompanying code snippet is shown as follows.
...
This pipeline chains four verbs. A set of succinct verbs makes the composition self-explanatory: (1) `index_by()` + `summarise()`: compute 50%, 80% and 95% quantiles for every hour; (2) `mutate()`: add time components that are extracted from the index; (3) `gather()`: melt different quantiles into a long form. Step (2) and (3) prepare the data for visualisation later. Every verb generates the output of the same type of the input, making chaining possible. Once the initial setup is done, the program takes care of updating the index and key, and empowers users concentrate on the substance of data analysis. Every one else who do not write the code would find the code fairly easy to comprehend.
