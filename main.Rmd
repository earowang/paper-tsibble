...
The **tidyverse** way of thinking largely formalizes exploratory data analysis. Tidy concepts have been extended to other domains in R community.

Temporal data is integrated to tidy data and the grammar. @r4ds argues that 80% of data analysis tasks can be solved with tidy tools while the remaining 20% requires other tools.

# Contextual semantics {#sec:semantics}

The choice of tidy representation of temporal data is made from a data-centric perspective, which is taken in the light of the operations that are to be performed on the data. Contextual semantics are introduced to tidy data in order to support more intuitive time-related manipulations and enlighten new perspectives for time series model inputs. Index, key and time interval are the three stone pillars to this new semantically-structured temporal data, which we name as "tsibble", inspired by "ts" and "tibble".
...
## Time-based pipeline {#sec:timepipeline}
...
\begin{table}[!hbt]
\caption{A test}\label{tab:verb-table}
\centering
\begin{tabular}{ lll }
\toprule
...
\multirow{4}{*}{Column-wise} & \textbf{\texttt{select()}} & Pick columns by variables \\
 ...
 & \textbf{\texttt{transmute()}} & Add new variables \\
 & \textbf{\texttt{summarise()}} & Aggregate values over time \\ \hline
\multirow{3}{*}{Group-wise} & \texttt{index\_by()} & Group by new time index \\
 & \textbf{\texttt{group\_by()}} & Group by one or more variables \\
 & \texttt{group\_by\_key()} & Group by key variables \\ \hline
\multirow{4}{*}{Reshape data} & \textbf{\texttt{gather()}} & David Batty \\
 & \textbf{\texttt{spread()}} & Eirik Bakke \\
 & \textbf{\texttt{nest()}} & Eirik Bakke \\
 & \textbf{\texttt{unnest()}} & Jody Morris \\ \hline
\multirow{6}{*}{Join data} & \textbf{\texttt{left\_join()}} & David Batty \\
 & \textbf{\texttt{right\_join()}} & Eirik Bakke \\
 & \textbf{\texttt{full\_join()}} & Eirik Bakke \\
 & \textbf{\texttt{inner\_join()}} & Jody Morris \\
 & \textbf{\texttt{semi\_join()}} & Jody Morris \\
 & \textbf{\texttt{anti\_join()}} & Jody Morris \\
\bottomrule
\end{tabular}
\end{table}
...
A set of verbs have been implemented to conduct temporal operations. There are three kinds of sliding window operations:
...
As the name indicates, `slide()` expects one input, `slide2()` two inputs, and `pslide()` multiple inputs. This family of window functions features functional programming. Any function can be mapped and rolled over a block of elements given a specified window size. For type stability, the functions always return lists. Other variants including `slide_lgl()`, `slide_int()`, `slide_dbl()`, `slide_chr()` return vectors of the corresponding type, as well as `slide_dfr()` and `slide_dfc()` for row-binding and column-binding data frames respectively. Their multiprocessing equivalents prefixed by `future_` enable rolling in parallel. This family of functions empowers users to incorporate window-related operations in their workflows.

# Software structure and design decisions {#sec:software}

Time series are represented as matrices, with standards being provided by native `ts` object in R, extended by **zoo** [@zoo2005], and **xts** [@R-xts]. As discussed in Section \ref{sec:time-series}, this data organization sets far apart from data origin. Due to time isolated from the main data, the supporting functions that intend to deal with time must be applied to whole data. That data-centric & human-centered.
...
The **tsibble** package provides an elegant solution to manage and manipulate medium-sized temporal data in memory.
