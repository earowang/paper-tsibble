...
(ref:map-airlines-cap) An overview of US airline traffic in 2017. Points indicate locations of airports, and lines the plane movements across the country. There are a few hubs connecting most airlines and sporadic airports.

The dataset of 2017 on-time performance for US domestic flights represents temporal-context data caught in the wild, sourced from US Bureau of Transportation Statistics (reference). The data, arrived in much greater resolution, includes many aspects and measurements of every single flight at its scheduled departure time from performance metrics to flight routes. Table ?? shows the millions of observations and 22 variables ranging from numerics to characters to date-times, which are intrinsically heterogeneous. Flights have their own scheduled departure times, resulting irregular space in time. This example also characterizes itself as event data, actions and states observed at the point in time.
...
The data easily fits into the tsibble framework without the loss of data richness. To declare it to a tsibble, the setup is to specify `sched_dep_datetime` as "index", `flight` variable as "key", and `regular = FALSE`. A rigorous check is performed for validating the distinct rows determined by key and index.
