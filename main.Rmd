...
A number of time classes are supported and easily extensible to new types. The base line is that time indices can be potentially ordered in past to future. Characters cannot be ordered, so a variable hosting characters will make it unsupported for index. Ordered factors, however, can be immediately recognized as valid index. For instance, month names have a natural ordering from January to December, and so are weekdays from Monday to Sunday.
