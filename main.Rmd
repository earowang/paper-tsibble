...
Current time series objects, which are model-oriented matrices with little support for non-numeric values, feed into the framework solely at the modelling stage. Analysts are expected to do their own data pre-processing and take care of anything else needed to get to this stage, which leads to a myriad ad-hoc solutions and duplicated efforts. The diagram correspondingly would place the model at the centre of the analytical universe, and all the transformations, visualisations would hinge on that format.
...
The **tidyverse** largely formalizes exploratory data analysis. The whole R community has quickly adopted the **tidyverse** way of thinking to much broader domains, such as simple features for spatial data in the **sf** package [@RJ-2018-009] and missing value handling in the **naniar** package [@tierney-naniar-2018]. Temporal data is left behind in this regard. This paper proposes a new tidy representation for temporal data and a unified framework to streamline the workflow from data pre-processing to visualization and forecasting, as an integral part of tidy data analysis.
...
Temporal data often involves moving window calculations. A set of verbs have been implemented to allow for different variations of sliding windows operations:
...
Matrices are a conventional way of organizing time series, with standards being provided by native `ts` object in R, extended by **zoo** [@zoo2005], and **xts** [@R-xts]. Time indices are stored as an attribute, separating from the main body of the data. 

data-centric & human-centered.
